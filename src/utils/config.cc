...
json DecConfig::loadCache(const std::string& network,
  ...
  channel_name += ".json";
  fs::path file =
      (*homeDir) / ".decirc" / "message-cache" / network / channel_name;
  if (!fs::exists(file)) {
    delete homeDir;
    return json::array();
  }
...
void DecConfig::saveCache(const std::string& network,
                          const std::string& channel,
                          json& list) {
  fs::path* homeDir = getHomeFolder();
  if (!homeDir) {
...
std::string DecConfig::getFallbackUserName() {
  load();
